...
      // NOTE: REPLACE 'pk.eyJ1...BHHnOsHQ' with your actual Mapbox Public Access Token
...
  const handleImmediateMapNavigation = useCallback(async (event, userLocation) => {
    try {
        const coordinate = event.geometry.coordinates; // [lng, lat]
        ...
        // 1. Create the destination object immediately
        const destination = {
            id: 'tap-' + Date.now(),
        ...
        // 3. Immediately start navigation
...
}, [validateCoordinates, startNavigation, setShowDestinationOptions]);
...
  const selectRandomDestination = useCallback(() => {
    // Generates a random coordinate globally
...
    startNavigation,
    stopNavigation,
    handleMapPress, // For modal/classic use
    handleImmediateMapNavigation, // For immediate tap-to-route
